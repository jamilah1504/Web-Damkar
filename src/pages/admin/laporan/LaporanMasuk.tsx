                        ...
                        >
                          ...
                          >
                            <IconButton
                              size="small"
                              sx={{ color: 'red' }}
                              onClick={() => handleHapusLaporan(laporan.id)}
                            >
                            ...
                            <IconButton
                              size="small"
                              sx={{ color: 'black' }}
                              onClick={() => handleLihatDetail(laporan)}
                            >
...
const AdminLaporanMasuk: React.FC = () => {
  ...
  const handleSubmitStatus = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/insiden/${currentInsiden.id}/status`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleEditLaporanFormChange = (
  ...
  const handleHapusDokumentasi = async (dokumentasiId: number) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(
        `http://localhost:5000/api/reports/dokumentasi/${dokumentasiId}`,
        {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        },
                ...
                {!isEditingLaporan ? (
                  ...
                  <>
                    <Typography variant="body1" gutterBottom>
                      ...
                      {selectedLaporan.namaPelapor ||
                        selectedLaporan.Pelapor?.name ||
                        'Pelapor Anonim'}
                    </Typography>
                    <Typography variant="body1" gutterBottom>
                    ...
                    <Typography variant="body1" gutterBottom>
                      ...
                      {selectedLaporan.alamatKejadian || 'Tidak ada alamat'}
                    </Typography>
                    ...
                    <Paper
                      variant="outlined"
                      sx={{ p: 2, bgcolor: '#f9f9f9', maxHeight: 150, overflowY: 'auto' }}
                    >
            ...
            <FormControl fullWidth sx={{ mt: 2 }}>
              <Autocomplete
                ...
                getOptionLabel={(option) =>
                  `${option.name} ${option.pangkat ? `(${option.pangkat})` : ''}`
                }
